// Plain Cordova (vanilla TS) — Applaud IQ embed with a tiny query-based router (no framework).
...
import { ApplaudIQ, type EmbedHandle } from '@applaudiq/embed-cordova';
  ...
  app.innerHTML = `
    <div class="aiq-app">
      <header class="aiq-hero">
        ...
        <p class="aiq-hero-sub">Cordova · vanilla — embed example</p>
      </header>
      <main class="aiq-content">
        ...
        <p class="aiq-footer">@applaudiq/embed-cordova · by Arulraj V</p>
      </main>
    </div>
  `;
...
// Cordova: wait for `deviceready` so the plugins (InAppBrowser / customurlscheme) are loaded before the
// first render. On plain web (no cordova.js) there's no such event — render immediately. `cordova.js` is
// injected into the built www/index.html for native targets by run-local.sh (kept out of the source so
// the plain `vite build` for web doesn't try to resolve it).
