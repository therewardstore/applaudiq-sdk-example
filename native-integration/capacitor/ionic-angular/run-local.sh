#!/usr/bin/env bash
# Build + run this Capacitor + Ionic Angular example against the LOCAL portal — on the web, an iOS
...
# Web: serve the dev build on :4200 (an allowed embed origin). Non-native — the embed shows reCAPTCHA and
# the web #aiq_sso SSO return is handled in the browser. No Capacitor sync/run needed.
