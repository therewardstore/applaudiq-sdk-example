# Applaud IQ — Capacitor + Vue 3 (plain)

A **Capacitor + Vue 3** app (Vite, no Ionic) embedding the Applaud IQ portal with
[`@applaudiq/embed-capacitor`](https://www.npmjs.com/package/@applaudiq/embed-capacitor) — Manual + Auto login + SSO.

Real **named routes** (plain `vue-router`) drive a Home → Embed flow:
...
routes back to Home there. Plain Vue has no router-level swipe gesture, so there's nothing to disable (unlike the
Ionic examples).
