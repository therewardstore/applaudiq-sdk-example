// Embed page — opens the full-screen embed for the requested mode, then returns to Home when the
// embed is dismissed. Reached via /embed?mode=manual|auto (auto carries its minted token in router
// state). The SDK renders a fixed full-viewport overlay over this view.
//
// Back chain: hardware Back (Android) / left-edge swipe (iOS) → SDK relays `applaudiq:back` → the
// portal steps back through its own history; at the embed root it replies `close` → the SDK tears
// the overlay down and fires `onClose` → we route back to Home here.
...
export class EmbedComponent implements OnInit, OnDestroy {
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (mode === 'auto') {
      ...
      // of /embed?mode=auto with no navigation state).
  ...
  ngOnDestroy(): void {
    // Leaving the route by any path (router.back, programmatic nav) tears the embed overlay down.
    this.handle?.close();
    this.handle = null;
  }
