# React (Vite) — ApplaudIQ Web SDK example

Embed the Applaud IQ recognition portal in a Vite + React SPA. The SDK loads once via `<script>` in
`index.html` (global `window.ApplaudIQ`); a `useApplaudIQ` hook mounts the embed inline. The app uses
**real route-based navigation** (`react-router-dom`) with a Home landing page and one route per login mode.
...
2. **Route** — `src/App.tsx` defines the router: `Layout` (top nav) wraps `/` (`routes/Home.tsx`),
   `/auto` (`routes/AutoLogin.tsx`), and `/manual` (`routes/ManualLogin.tsx`).
...
- A **publishable key** (`pk_live_…` / `pk_test_…`) from **HR portal → Settings → Embed SDK Keys** — needed
  in **both** modes. Add this example's origin **`http://localhost:5173`** to the key's allowed origins.
...
`open({ mode: 'manual' })` (via the `useApplaudIQ` hook in `src/useApplaudIQ.ts`):
...
Open **http://localhost:5173** and navigate **Home (`/`) → Auto-login (`/auto`) → Manual login (`/manual`)**
...
- **Blank frame / won't load** — your origin isn't on the key's allowed list. Add `http://localhost:5173`.
